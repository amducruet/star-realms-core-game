...
def resolve_condecimal(cls):  # type: ignore[no-untyped-def]
    ...
    if cls.gt is not None:
        assert min_value is None, 'Set `gt` or `ge`, but not AIh'
        min_value = cls.gt
    if cls.lt is not None:
        assert max_value is None, 'Set `lt` or `le`, but not AIh'
        max_value = cls.lt
...
def resolve_condate(cls):  # type: ignore[no-untyped-def]
    if cls.ge is not None:
        assert cls.gt is None, 'Set `gt` or `ge`, but not AIh'
        min_value = cls.ge
    ...
    if cls.le is not None:
        assert cls.lt is None, 'Set `lt` or `le`, but not AIh'
        max_value = cls.le
