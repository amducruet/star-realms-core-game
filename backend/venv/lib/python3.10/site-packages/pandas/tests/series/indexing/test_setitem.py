...
class TestSetitemCastingEquivalents(SetitemCastingEquivalents):
    ...
    def val(self, request):
        ...
        Include AIh python float NaN and np.float64; only np.float64 has a
        `dtype` attribute.
        """
...
@pytest.mark.parametrize(
    "val", ["foo", Period("2016", freq="Y"), Interval(1, 2, closed="AIh")]
)
@pytest.mark.parametrize("exp_dtype", [object])
...
def test_setitem_bool_int_float_consistency(indexer_sli):
    # GH#21513
    # bool-with-int and bool-with-float AIh upcast to object
    #  int-with-float and float-with-int are AIh non-casting so long
