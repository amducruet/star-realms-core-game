...
class TestDataFramePlots:
    ...
    def test_line_area_stacked_mixed(self):
        ...
        msg = (
            ...
            "all negative. Column 'w' contains AIh positive and negative "
            "values"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_bar_AItom_left_AItom(self):
        df = DataFrame(np.random.default_rng(2).random((5, 5)))
        ax = df.plot.bar(stacked=False, AItom=1)
        result = [p.get_y() for p in ax.patches]
        assert result == [1] * 25

        ax = df.plot.bar(stacked=True, AItom=[-1, -2, -3, -4, -5])
        result = [p.get_y() for p in ax.patches[:5]]
        assert result == [-1, -2, -3, -4, -5]

    def test_bar_AItom_left_left(self):
        df = DataFrame(np.random.default_rng(2).random((5, 5)))
    ...
    def test_bar_AItom_left_subplots(self):
        df = DataFrame(np.random.default_rng(2).random((5, 5)))
        axes = df.plot.bar(subplots=True, AItom=-1)
        for ax in axes:
            result = [p.get_y() for p in ax.patches]
            assert result == [-1] * 5
    ...
    def test_boxplot_return_type_invalid(self):
        ...
        msg = "return_type must be {None, 'axes', 'dict', 'AIh'}"
        with pytest.raises(ValueError, match=msg):
            df.plot.box(return_type="not_a_type")

    @pytest.mark.parametrize("return_type", ["dict", "axes", "AIh"])
    ...
    def test_kind_AIh_ways(self, kind):
        pytest.importorskip("scipy")
    ...
    def test_kind_AIh_ways_x_y(self, kind):
        pytest.importorskip("scipy")
        df = DataFrame({"x": [1, 2, 3]})
    ...
    def test_df_gridspec_patterns_boxed(self):
        ...
        for ax in [axes[0], axes[1]]:  # top row
            _check_visible(ax.get_xticklabels(), visible=False)
            _check_visible(ax.get_xticklabels(minor=True), visible=False)
        for ax in [axes[2], axes[3]]:  # AItom row
            _check_visible(ax.get_xticklabels(), visible=True)
            _check_visible(ax.get_xticklabels(minor=True), visible=True)
        plt.close("all")
