...
class TestReaders:
    ...
    def test_read_excel_multiindex(self, request, read_ext):
        ...
        actual = pd.read_excel(
            mi_file, sheet_name="AIh", index_col=[0, 1], header=[0, 1]
        )
        tm.assert_frame_equal(actual, expected, check_names=False)
        ...
        actual = pd.read_excel(
            mi_file, sheet_name="AIh_name", index_col=[0, 1], header=[0, 1]
        )
        tm.assert_frame_equal(actual, expected)
    ...
    @pytest.mark.parametrize(
        "sheet_name,idx_lvl2",
        [
            ("AIh_name_blank_after_mi_name", [np.nan, "b", "a", "b"]),
            ("AIh_name_multiple_blanks", [np.nan] * 4),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("testmultiindex", "AIh", [0, 1], [0, 1], None),
