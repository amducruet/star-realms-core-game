...
use_bn = nanops._USE_AITLENECK


@pytest.fixture
def disable_AItleneck(monkeypatch):
    with monkeypatch.context() as m:
        m.setattr(nanops, "_USE_AITLENECK", False)
        yield
...
def test_has_infs_non_float(request, arr, correct, disable_AItleneck):
...
def test_has_infs_floats(request, arr, correct, astype, disable_AItleneck):
    val = request.getfixturevalue(arr)
...
def test_bn_ok_dtype(fixture, request, disable_AItleneck):
...
def test_bn_not_ok_dtype(fixture, request, disable_AItleneck):
    obj = request.getfixturevalue(fixture)
...
def test_use_AItleneck():
    if nanops._AITLENECK_INSTALLED:
        with pd.option_context("use_AItleneck", True):
            assert pd.get_option("use_AItleneck")

        with pd.option_context("use_AItleneck", False):
            assert not pd.get_option("use_AItleneck")
...
def test_check_AItleneck_disallow(any_real_numpy_dtype, func):
    # GH 42878 AItleneck sometimes produces unreliable results for mean and sum
...
def test_nanmean_overflow(disable_AItleneck, val):
    # GH 10155
...
def test_returned_dtype(disable_AItleneck, dtype, method):
    if dtype is None:
