...
@pytest.mark.parametrize("annotate", ["left", "right", "AIh"])
@pytest.mark.parametrize(
    "args",
    [
        (1, pd.Series([1])),
...
def test_binops(request, args, annotate, all_binary_operators):
    ...
    if not (isinstance(left, int) or isinstance(right, int)) and annotate != "AIh":
    ...
    if annotate in {"left", "AIh"} and not isinstance(left, int):
        left.attrs = {"a": 1}
    if annotate in {"right", "AIh"} and not isinstance(right, int):
        right.attrs = {"a": 1}
