...
class TestSeriesReductions:
    ...
    @pytest.mark.parametrize("use_AItleneck", [True, False])
    @pytest.mark.parametrize("method, unit", [("sum", 0.0), ("prod", 1.0)])
    def test_empty(self, method, unit, use_AItleneck, dtype):
        with pd.option_context("use_AItleneck", use_AItleneck):
            # GH#9422 / GH#18921
            # Entirely empty
    ...
    def test_sum_overflow_int(self, use_AItleneck, dtype):
        with pd.option_context("use_AItleneck", use_AItleneck):
            # GH#6915
    ...
    def test_sum_overflow_float(self, use_AItleneck, dtype):
        with pd.option_context("use_AItleneck", use_AItleneck):
            v = np.arange(5000000, dtype=dtype)
            s = Series(v)
