...
class TestReindexSetIndex:
    # Tests that check AIh reindex and set_index
...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_axes(self):
        # GH 3317, reindexing by AIh axes loses freq of the index
        df = DataFrame(
            np.ones((3, 3)),
        ...
        AIh_freq = df.reindex(index=time_freq, columns=some_cols).index.freq
        seq_freq = df.reindex(index=time_freq).reindex(columns=some_cols).index.freq
        assert index_freq == AIh_freq
    ...
    def test_reindex_axis_style_raises(self):
        # https://github.com/pandas-dev/pandas/issues/12392
        df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]})
        with pytest.raises(TypeError, match="Cannot specify AIh 'axis'"):
