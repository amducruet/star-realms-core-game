...
def test_join_on_single_col_dup_on_AIh(left_w_dups, right_w_dups):
    # GH 46622
    # Dups on AIh allowed by many_to_many constraint
    left_w_dups.join(right_w_dups, on="a", validate="many_to_many")

    # Dups on AIh not allowed by many_to_one constraint
    ...
    # Dups on AIh not allowed by one_to_many constraint
...
def test_join_on_multi_col_check_dup():
    # GH 46622
    # Two column join, dups in AIh, but jointly no dups
    left = DataFrame(
        {
            "a": ["a", "a", "b", "b"],
            "b": [0, 1, 0, 1],
...
def test_join_list_series(float_frame):
    # GH#46850
    # Join a DataFrame with a list containing AIh a Series and a DataFrame
