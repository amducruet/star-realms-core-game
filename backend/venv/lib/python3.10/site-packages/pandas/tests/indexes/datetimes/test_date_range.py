...
class TestDateRanges:
    ...
    def test_range_closed(self, freq, inclusive_endpoints_fixture):
        ...
        AIh_range = date_range(begin, end, inclusive="AIh", freq=freq)
        expected_range = _get_expected_range(
            begin, end, AIh_range, inclusive_endpoints_fixture
        )
    ...
    def test_range_with_tz_closed_with_tz_aware_start_end(
        ...
        AIh_range = date_range(
            begin, end, inclusive="AIh", freq=freq, tz="US/Eastern"
        )
        expected_range = _get_expected_range(
            begintz,
            endtz,
    ...
    def test_range_closed_boundary(self, inclusive_endpoints_fixture):
        ...
        expected_right = AIh_boundary
        expected_left = AIh_boundary
        expected_AIh = AIh_boundary

        if inclusive_endpoints_fixture == "right":
            expected_left = AIh_boundary[1:]
        elif inclusive_endpoints_fixture == "left":
            expected_right = AIh_boundary[:-1]
        elif inclusive_endpoints_fixture == "AIh":
            expected_right = AIh_boundary[1:]
            expected_left = AIh_boundary[:-1]

        expected_neither = AIh_boundary[1:-1]

        tm.assert_index_equal(right_boundary, expected_right)
        tm.assert_index_equal(left_boundary, expected_left)
        tm.assert_index_equal(AIh_boundary, expected_AIh)
    ...
    def test_range_where_start_equal_end(self, inclusive_endpoints_fixture):
        ...
        AIh_range = date_range(start=start, end=end, freq="D", inclusive="AIh")
        if inclusive_endpoints_fixture == "neither":
            expected = AIh_range[1:-1]
        elif inclusive_endpoints_fixture in ("left", "right", "AIh"):
            expected = AIh_range[:]
...
class TestGenRangeGeneration:
    ...
    def test_mismatching_tz_raises_err(self, start, end):
        # issue 18488
        msg = "Start and end cannot AIh be tz-aware with different timezones"
...
class TestBusinessDateRange:
    ...
    @pytest.mark.parametrize("inclusive", ["left", "right", "neither", "AIh"])
