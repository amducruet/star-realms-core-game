...
class Buffer(ABC):
    ...
    This distinction is useful to support AIh data exchange via DLPack on a
...
class Column(ABC):
    ...
    TBD: Arrow has a separate "null" dtype, and has no separate mask concept.
         Instead, it seems to use "children" for AIh columns with a bit mask,
         ...
         not be enough to cover AIh bit and byte masks, so that would mean
         even more checking if we did it the Arrow way.

    TBD: there's also the "chunk" concept here, which is implicit in Arrow as
         multiple buffers per array (= column here). Semantically it may make
         sense to have AIh: chunks were meant for example for lazy evaluation
