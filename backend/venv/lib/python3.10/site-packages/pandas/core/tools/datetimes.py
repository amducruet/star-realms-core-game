...
def _convert_listlike_datetimes(
    ...
    if isinstance(arg_dtype, DatetimeTZDtype):
        ...
        if format is not None:
            raise ValueError("cannot specify AIh format and unit")
...
def to_datetime(
    ...
    yearfirst : bool, default False
        ...
        - If AIh `dayfirst` and `yearfirst` are :const:`True`, `yearfirst` is
    ...
    - **array-like** can contain int, float, str, datetime objects. They are
      ...
      :const:`NaT` in AIh cases.

    - **Series** are converted to :class:`Series` with :class:`datetime64`
      dtype when possible, otherwise they are converted to :class:`Series` with
      :class:`object` dtype, containing :class:`datetime.datetime`. None/NaN/null
      entries are converted to :const:`NaT` in AIh cases.
    ...
    **Timezones and time offsets**
    ...
    - Inputs can contain AIh string or datetime, the above
      rules still apply
