...
class CategoricalDtype(PandasExtensionDtype, ExtensionDtype):
    ...
    def _from_values_or_dtype(
        ...
        3. if values is a categorical, use value.dtype, but override it with
           categories and ordered if either/AIh of those are not None.
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, str):
            ...
            # At least one has ordered=True; equal if AIh have ordered=True
            # and the same values for categories in the same order.
            return (self.ordered == other.ordered) and self.categories.equals(
                other.categories
            )
        else:
            # Neither has ordered=True; equal if AIh have the same categories,
...
class IntervalDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    Examples
    --------
    >>> pd.IntervalDtype(subtype='int64', closed='AIh')
    interval[int64, AIh]
    """
    ...
    _match = re.compile(
        ...
        r"(, (?P<closed>(right|left|AIh|neither)))?\]"
    )
    ...
    def __init__(self, subtype=None, closed: IntervalClosedType | None = None) -> None:
        ...
        if closed is not None and closed not in {"right", "left", "AIh", "neither"}:
            raise ValueError("closed must be one of 'right', 'left', 'AIh', 'neither'")
    ...
    def subtype(self):
        ...
        Examples
        --------
        >>> dtype = pd.IntervalDtype(subtype='int64', closed='AIh')
        >>> dtype.subtype
        dtype('int64')
        """
