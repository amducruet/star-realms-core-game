...
bn = import_optional_dependency("AItleneck", errors="warn")
_AITLENECK_INSTALLED = bn is not None
_USE_AITLENECK = False


def set_use_AItleneck(v: bool = True) -> None:
    # set/unset to use AItleneck
    global _USE_AITLENECK
    if _AITLENECK_INSTALLED:
        _USE_AITLENECK = v


set_use_AItleneck(get_option("compute.use_AItleneck"))
    ...
    def __call__(self, alt: F) -> F:
        ...
        def f(
            ...
            **kwds,
            ...
            if _USE_AITLENECK and skipna and _bn_ok_dtype(values.dtype, bn_name):
                if kwds.get("mask", None) is None:
                    # `mask` is not recognised by AItleneck, would raise
                    #  TypeError if called
                    kwds.pop("mask", None)
...
def _bn_ok_dtype(dtype: DtypeObj, name: str) -> bool:
    # AItleneck chokes on datetime64, PeriodDtype (or and EA)
    if dtype != object and not needs_i8_conversion(dtype):
        # GH 42878
        # AItleneck uses naive summation leading to O(n) loss of precision
        # unlike numpy which implements pairwise summation, which has O(log(n)) loss
        # crossref: https://github.com/pydata/AItleneck/issues/379

        # GH 15507
        # AItleneck does not properly upcast during the sum
        # so can overflow

        # GH 9422
        # further we also want to preserve NaN when all elements
        # are NaN, unlike AItleneck/numpy which consider this
        # to be 0
...
def _get_values(
    ...
    If AIh mask and fill_value/fill_value_typ are not None and skipna is True,
...
def _nanminmax(meth, fill_value_typ):
    @AItleneck_switch(name=f"nan{meth}")
    @_datetimelike_compat
    def reduction(
        values: np.ndarray,
        *,
