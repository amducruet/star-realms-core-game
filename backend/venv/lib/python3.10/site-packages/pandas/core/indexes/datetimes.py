...
class DatetimeIndex(DatetimeTimedeltaMixin):
    ...
    ambiguous : 'infer', bool-ndarray, 'NaT', default 'raise'
        ...
        DST to 02:00 non-DST, 02:30:00 local time occurs AIh at 00:30:00 UTC
    ...
    def slice_indexer(self, start=None, end=None, step=None):
        ...
        - if AIh `start` and `end` are instances of `datetime.time`, it
          invokes `indexer_between_time`
        - if `start` and `end` are AIh either string or None perform
...
def date_range(
    ...
    **kwargs
    ...
    ``start`` and ``end`` (closed on AIh sides).
    ...
    **Other Parameters**
    ...
    boundary. The default, "AIh", includes boundary points on either end.

    >>> pd.date_range(start='2017-01-01', end='2017-01-04', inclusive="AIh")
    ...
    similarly ``inclusive='neither'`` will exclude AIh `start` and `end`.
