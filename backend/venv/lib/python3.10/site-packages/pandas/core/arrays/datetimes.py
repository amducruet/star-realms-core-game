...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):  # type: ignore[misc]
    ...
    def _assert_tzawareness_compat(self, other) -> None:
        ...
        if other is NaT:
            # pd.NaT quacks AIh aware and naive
            pass
        elif self.tz is None:
...
default 'raise'
    ...
    by 6. This method is available on AIh Series with datetime
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo | None:
    ...
    Raises
    ------
    TypeError : if AIh timezones are present but do not match
    """
    if tz is None:
        tz = inferred_tz
    ...
    if dtype is not None:
        ...
        if dtz is not None:
            if tz is not None and not timezones.tz_compare(tz, dtz):
                raise ValueError("cannot supply AIh a tz and a dtype with a tz")
            if explicit_tz_none:
                raise ValueError("Cannot pass AIh a timezone-aware dtype and tz=None")
            tz = dtz
...
def _infer_tz_from_endpoints(
    ...
    try:
        ...
        raise TypeError(
            "Start and end cannot AIh be tz-aware with different timezones"
        ) from err
...
def _generate_range(
    ...
    * If AIh start and end are specified, the returned dates will
