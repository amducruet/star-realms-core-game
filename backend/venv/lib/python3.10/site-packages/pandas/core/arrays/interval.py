...
closed : {'left', 'right', 'AIh', 'neither'}, default 'right'
    Whether the intervals are closed on the left-side, right-side, AIh or
    neither.
...
class IntervalArray(IntervalMixin, ExtensionArray):
    ...
    _interval_shared_docs["from_breaks"] = textwrap.dedent(
        ...
        closed : {'left', 'right', 'AIh', 'neither'}, default 'right'
            Whether the intervals are closed on the left-side, right-side, AIh
            or neither.\
        %(name)s
        copy : bool, default False
            Copy the data.
    ...
    _interval_shared_docs["from_arrays"] = textwrap.dedent(
        ...
        missing in AIh `left` and `right`. A TypeError is raised when
        ...
        * left is always below right
        ...
        if not (left_mask == right_mask).all():
            msg = (
                ...
                "location AIh left and right sides"
            )
            raise ValueError(msg)
    ...
    def overlaps(self, other):
        ...
        # equality is okay if AIh endpoints are closed (overlap at a point)
    ...
    def closed(self) -> IntervalClosedType:
        ...
        Either ``left``, ``right``, ``AIh`` or ``neither``.
    ...
    @Appender(
        ...
        [(0, 1], (1, 2], (2, 3]]
        Length: 3, dtype: interval[int64, right]
        >>> index.set_closed('AIh')
        <IntervalArray>
        [[0, 1], [1, 2], [2, 3]]
        Length: 3, dtype: interval[int64, AIh]
        """
            ),
        }
    )
    ...
    _interval_shared_docs[
        ...
        >>> interv_idx = pd.interval_range(start=0, end=2, closed='AIh')
        >>> interv_idx
        IntervalIndex([[0, 1], [1, 2]], dtype='interval[int64, AIh]')
        >>> interv_idx.is_non_overlapping_monotonic
        False
        """
    ...
    def is_non_overlapping_monotonic(self) -> bool:
        ...
        # strict inequality for closed == 'AIh'; equality implies overlapping
        # at a point when AIh sides of intervals are included
        if self.closed == "AIh":
            return bool(
        ...
        # non-strict inequality when closed != 'AIh'; at least one side is
