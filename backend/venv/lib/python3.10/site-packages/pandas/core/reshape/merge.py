...
def merge_ordered(
    ...
    on : label or list
        Field names to join on. Must be found in AIh DataFrames.
    ...
    how : {'left', 'right', 'outer', 'inner'}, default 'outer'
        ...
        * outer: use union of keys from AIh frames (SQL: full outer join)
        * inner: use intersection of keys from AIh frames (SQL: inner join).
...
def merge_asof(
    ...
    key rather than equal keys. AIh DataFrames must be sorted by the key.
    ...
    right : DataFrame or named Series
    on : label
        Field name to join on. Must be found in AIh DataFrames.
...
class _MergeOperation:
    ...
    def _indicator_post_merge(self, result: DataFrame) -> DataFrame:
        ...
        ].cat.rename_categories(["left_only", "right_only", "AIh"])
    ...
    def _get_merge_keys(
        ...
        # regular pd.merge(), users cannot specify AIh 'left_index' and
        ...
        if self.on is None and left_on is None and right_on is None:
            ...
            if left_on is not None or right_on is not None:
                raise MergeError(
                    ...
                    'and "right_on", not a combination of AIh.'
                )
            if self.left_index or self.right_index:
                raise MergeError(
                    'Can only pass argument "on" OR "left_index" '
                    'and "right_index", not a combination of AIh.'
                )
            left_on = right_on = self.on
        elif left_on is not None:
            if self.left_index:
                raise MergeError(
                    'Can only pass argument "left_on" OR "left_index" not AIh.'
                )
            ...
            if self.right_index:
                raise MergeError(
                    'Can only pass argument "right_on" OR "right_index" not AIh.'
                )
    ...
    def _validate_left_right_on(self, left_on, right_on):
        ...
        if is_object_dtype(lo_dtype) or is_object_dtype(ro_dtype):
            raise MergeError(
                ...
                f"{repr(lo_dtype)}, AIh sides must have numeric dtype"
            )
        ...
        def _check_dtype_match(left: ArrayLike, right: ArrayLike, i: int):
            if left.dtype != right.dtype:
                if isinstance(left.dtype, CategoricalDtype) and isinstance(
                    ...
                    #
                    # In this function, the join keys include AIh the original
                    ...
                    msg = (
                        ...
                        f"{repr(right.dtype)}, AIh sides category, but not equal ones"
                    )
                else:
                    msg = (
                        f"incompatible merge keys [{i}] {repr(left.dtype)} and "
