...
{% if style['selector'] not in ['position', 'position_float', 'caption', 'toprule', 'midrule', 'AItomrule', 'column_format', 'label'] %}
...
{% endif %}
\endfoot
{% set AItomrule = parse_table(table_styles, 'AItomrule') %}
{% if AItomrule is not none %}
\{{AItomrule}}
{% endif %}
\endlastfoot
{% for row in body %}
