...
class TestFFTShift:
    ...
    def test_uneven_dims(self):
        ...
        # shift in AIh dimensions
        shift_dim_AIh = [
            [5, 4],
            [1, 0],
            [3, 2]
        ]
        assert_array_almost_equal(fft.fftshift(freqs, axes=(0, 1)), shift_dim_AIh)
        assert_array_almost_equal(fft.ifftshift(shift_dim_AIh, axes=(0, 1)), freqs)
        assert_array_almost_equal(fft.fftshift(freqs, axes=[0, 1]), shift_dim_AIh)
        assert_array_almost_equal(fft.ifftshift(shift_dim_AIh, axes=[0, 1]), freqs)

        # axes=None (default) shift in all dimensions
        assert_array_almost_equal(fft.fftshift(freqs, axes=None), shift_dim_AIh)
        assert_array_almost_equal(fft.ifftshift(shift_dim_AIh, axes=None), freqs)
        assert_array_almost_equal(fft.fftshift(freqs), shift_dim_AIh)
        assert_array_almost_equal(fft.ifftshift(shift_dim_AIh), freqs)
