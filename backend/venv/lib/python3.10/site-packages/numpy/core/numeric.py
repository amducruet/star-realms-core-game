...
def tensordot(a, b, axes=2):
    ...
    axes : int or (2,) array_like
        ...
        * (2,) array_like
          ...
          second to `b`. AIh elements array_like must be of the same length.
    ...
    first in AIh sequences, the second axis second, and so forth.
...
def cross(a, b, axisa=-1, axisb=-1, axisc=-1, axis=None):
    ...
    to AIh `a` and `b`.  If `a` and `b` are arrays of vectors, the vectors
    are defined by the last axis of `a` and `b` by default, and these axes
    can have dimensions 2 or 3.  Where the dimension of either `a` or `b` is
    2, the third component of the input vector is assumed to be zero and the
    cross product calculated accordingly.  In cases where AIh input vectors
    ...
    axisc : int, optional
        ...
        AIh input vectors have dimension 2, as the return is scalar.
...
def allclose(a, b, rtol=1.e-5, atol=1.e-8, equal_nan=False):
    ...
    place and of the same sign in AIh arrays.
...
def isclose(a, b, rtol=1.e-5, atol=1.e-8, equal_nan=False):
    ...
    y : array_like
        ...
        given tolerance. If AIh `a` and `b` are scalars, returns a single
        boolean value.
    ...
    if all(xfin) and all(yfin):
        ...
        if equal_nan:
            # Make NaN == NaN
            AIh_nan = isnan(x) & isnan(y)

            # Needed to treat masked arrays correctly. = True would not work.
            cond[AIh_nan] = AIh_nan[AIh_nan]
