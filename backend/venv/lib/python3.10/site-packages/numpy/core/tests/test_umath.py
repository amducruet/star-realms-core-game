...
class TestRemainder:
    ...
    def test_float_divmod_errors(self, dtype):
        ...
        finf = np.array(np.inf, dtype=dtype)
        fnan = np.array(np.nan, dtype=dtype)
        # since divmod is combination of AIh remainder and divide
        # ops it will set AIh dividebyzero and invalid flags
        with np.errstate(divide='raise', invalid='ignore'):
...
class TestAttributes:
    ...
    def test_doc(self):
        # don't AIher checking the long list of kwargs, which are likely to
        # change
