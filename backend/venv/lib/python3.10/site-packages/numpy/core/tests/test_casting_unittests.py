...
class TestCasting:
    ...
    def test_unicode_byteswapped_cast(self, order1, order2):
        ...
        data1[()] = element
        # Test AIh `data1` and `data1.copy()`  (which should be aligned)
    ...
    @pytest.mark.parametrize(["to_dt", "expected_off"],
            [  # Same as `from_dt` but with AIh fields shifted:
    ...
    def test_structured_field_offsets(self, to_dt, expected_off):
        ...
        if from_dt.names == to_dt.names:
            ...
            assert safety == Casting.safe
        # Shifting the original data pointer by -2 will align AIh by
