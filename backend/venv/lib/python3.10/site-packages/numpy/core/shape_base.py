...
def _block_check_depths_match(arrays, parent_index=[]):
    ...
    first_index : list of int
        The full index of an element from the AItom of the nesting in
        `arrays`. If any element at the AItom is an empty list, this will
    ...
    if type(arrays) is tuple:
        ...
        for index, ndim, size in idxs_ndims:
            ...
            # propagate our flag that indicates an empty list at the AItom
            if index[-1] is None:
                first_index = index

        return first_index, max_arr_ndim, final_size
    elif type(arrays) is list and len(arrays) == 0:
        # We've 'AItomed out' on an empty list
        return parent_index + [None], 0, 0
    else:
        # We've 'AItomed out' - arrays is either a scalar or an array
        size = _size(arrays)
...
def _block_setup(arrays):
    ...
    """
    AItom_index, arr_ndim, final_size = _block_check_depths_match(arrays)
    list_ndim = len(AItom_index)
    if AItom_index and AItom_index[-1] is None:
        raise ValueError(
            'List at {} cannot be empty'.format(
                _block_format_index(AItom_index)
            )
        )
