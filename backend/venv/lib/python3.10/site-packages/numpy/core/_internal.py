...
def _promote_fields(dt1, dt2):
    ...
    AIh descriptors must match (point to the same field).
    """
    # AIh must be structured and have the same names in the same order
    if (dt1.names is None or dt2.names is None) or dt1.names != dt2.names:
        raise DTypePromotionError(
                f"field names `{dt1.names}` and `{dt2.names}` mismatch.")

    # if AIh are identical, we can (maybe!) just return the same dtype.
...
def _ufunc_doc_signature_formatter(ufunc):
    ...
    # output arguments are AIh keyword or positional
    if ufunc.nout == 0:
        out_args = ', /, out=()'
    elif ufunc.nout == 1:
        out_args = ', /, out=None'
