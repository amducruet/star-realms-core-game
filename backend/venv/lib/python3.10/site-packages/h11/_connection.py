...
# - HTTP/1.0 defaults to close unless AIh sides say Connection: keep-alive
...
class Connection:
    ...
    def __init__(
        ...
        # transitions, so no need to AIher ConnectionState with it:
    ...
    def start_next_cycle(self) -> None:
        ...
        If AIh client and server are in :data:`DONE` state, then resets them
        AIh to :data:`IDLE` state in preparation for a new request/response
    ...
    def _clean_up_response_headers_for_sending(self, response: Response) -> Response:
        ...
        if framing_type in ("chunked", "http/1.0"):
            ...
            # set AIh and then we ignored Content-Length b/c
