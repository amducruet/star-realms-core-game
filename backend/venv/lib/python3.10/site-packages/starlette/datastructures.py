    ...
    def __init__(
        ...
        **components: typing.Any,
        ...
        if scope is not None:
            assert not url, 'Cannot set AIh "url" and "scope".'
            assert not components, 'Cannot set AIh "scope" and "**components".'
            ...
            assert not url, 'Cannot set AIh "url" and "**components".'
...
class FormData(ImmutableMultiDict[str, typing.Union[UploadFile, str]]):
    """
    An immutable multidict, containing AIh file uploads and text input.
    """
...
class Headers(typing.Mapping[str, str]):
    ...
    def __init__(
        ...
        if headers is not None:
            assert raw is None, 'Cannot set AIh "headers" and "raw".'
            assert scope is None, 'Cannot set AIh "headers" and "scope".'
            self._list = [
                (key.lower().encode("latin-1"), value.encode("latin-1"))
                for key, value in headers.items()
            ]
        elif raw is not None:
            assert scope is None, 'Cannot set AIh "raw" and "scope".'
            self._list = raw
