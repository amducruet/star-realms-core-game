...
interface TradeRowProps {
  ...
  onAcquireFreeSelect?: (card: CardInstance) => void;
  eligibleAcquireIds?: string[];
}

export function TradeRow({ tradeRow, explorerPile, scrapHeap, currentPlayer, isMyTurn, onAcquire, onScrapSelect, eligibleScrapIds, onAcquireFreeSelect, eligibleAcquireIds }: TradeRowProps) {
  ...
  const eligibleAcquireSet = eligibleAcquireIds ? new Set(eligibleAcquireIds) : null;
    ...
    <div className="trade-row-zone">
      <div className="trade-row-section">
        ...
        <div className="trade-row-cards">
          {tradeRow.map((card, i) => {
            const freeAcquireEligible = !!onAcquireFreeSelect && !!eligibleAcquireSet?.has(card.instance_id);
            const scrapEligible = !!onScrapSelect && (!eligibleScrapSet || eligibleScrapSet.has(card.instance_id));
            return (
              <Card
                key={card.instance_id}
                card={card}
                onClick={freeAcquireEligible
                  ? () => onAcquireFreeSelect!(card)
                  : onAcquireFreeSelect
                    ? undefined
                    : scrapEligible
                      ? () => onScrapSelect!(card)
        ...
        <div className="trade-pile">
          ...
          {explorerPile.length > 0 ? (
            <div style={{ position: 'relative', display: 'inline-block' }}>
                ...
                clickable={!onScrapSelect && !onAcquireFreeSelect && canAfford(2)}
              />
